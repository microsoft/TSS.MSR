...
typedef void(*TpmResponseCallbackHandler)(ByteVec tpmCommand,
        ByteVec tpmResponse, void *context);
...
class _DLLEXP_ Tpm2 {
    public:
        ...
        /// explicit sessions or use AUTH_SESSION::PWAP() to use a PWAP session with
        /// the auth-value in the associated handle.</summary>
        Tpm2& _Sessions(vector<AUTH_SESSION> sessions);
        ...
        ByteVec _GetRandLocal(UINT32 numBytes) {
    ...
    protected:
        ...
        ByteVec GetRandomBytes(size_t _numBytes);
        void Init();
        ...
        void GetAuthSessions(ByteVec& bufToFill, 
                             TPM_CC command, 
                             ByteVec commandParms,
                             int numAuthHandles,
                             vector<TPM_HANDLE *> handles);

        bool ProcessResponseSessions(ByteVec& sessionBuf,
                                     TPM_CC command,
                                     TPM_RC response,
