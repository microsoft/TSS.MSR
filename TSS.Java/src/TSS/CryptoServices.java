...
public class CryptoServices {
	...
	/**
	 * Return the size in bytes of a hash algorithm based on TPM_ALG_ID.Value
	 * 
	 * @param algVal The hash algorithm value
	 * @return The size of the hash output in bytes
	 */
	public static int hashLen(int algVal) {
		switch (algVal) {
		case 4:
			return 20;
		case 0xb:
...
/*
	...
	public static boolean validateQuote(TPMT_PUBLIC pubKey, PCR_ReadResponse expectedPcrs, byte[] nonce, QuoteResponse quote)
		{
			...
			TPM_ALG_ID hashAlg = CryptoServices.getSigningHashAlg(pubKey);
		    byte[] pcrHash = CryptoServices.hash(hashAlg,  pcrBuf.getBuf());
		    ...
		    Boolean quoteOk = CryptoServices.validateSignature(pubKey,  signedBlob,  quote.signature);
		    return quoteOk;
		}
	 ...
	 *
	 */
	public static class ActivationCredentialBundle 
	{
		public ActivationCredentialBundle()
		{
	 ...
	 */
	public static ActivationCredentialBundle createActivationCredential(
			TPMT_PUBLIC ek, 
			...
			byte[] secret) 
	{
		...
		CryptoServices.ActivationCredentialBundle act = new CryptoServices.ActivationCredentialBundle();
		...
		byte[] seed = Helpers.getRandom(digestSize(nameAlg));
		byte[] encSeed = oaepEncrypt(ekParms, ekPubKey, seed, schemeAlg, "IDENTITY");
		act.Secret = encSeed;

		byte[] lengthPrependedSecret = (new TPM2B_DIGEST(secret)).toTpm();
		byte[] symKey = KDFa(nameAlg, seed, "STORAGE", nameOfKeyToBeActivated, new byte[0], symmKeySize);
		...
		int hmacKeyBits = digestSize(nameAlg);
		byte[] hmacKey = KDFa(nameAlg, seed, "INTEGRITY", new byte[0], new byte[0], hmacKeyBits * 8);

		byte[] toHash = Helpers.concatenate(act.CredentialBlob.encIdentity, nameOfKeyToBeActivated);
		
		byte[] outerHmac = hmac(nameAlg, hmacKey, toHash);
		act.CredentialBlob.integrityHMAC = outerHmac;
		return act;

	}

	public static DuplicationBlob createImportableObject(
			...
			TPMT_SYM_DEF_OBJECT innerWrapper)
	{
		...
		DuplicationBlob blob = new CryptoServices.DuplicationBlob();
		...
		} else 
		{
			...
			byte[] innerIntegrity = Helpers.byteArrayToLenPrependedByteArray(CryptoServices.hash(nameAlg, toHash));
			...
			encryptedSensitive = CryptoServices.cfbEncrypt(true,TPM_ALG_ID.AES,innerWrapperKey,nullVec,innerData);
		...
		byte[] symmKey = CryptoServices.KDFa(targetParent.nameAlg,seed,"STORAGE",_publicPart.getName(),nullVec,newParentSymmKeyLen);

		byte[] dupSensitive = CryptoServices.cfbEncrypt(true,TPM_ALG_ID.AES,symmKey,nullVec,encryptedSensitive);

		int npNameNumBits = CryptoServices.digestSize(nameAlg) * 8;
		byte[] hmacKey = CryptoServices.KDFa(nameAlg, seed, "INTEGRITY", nullVec, nullVec, npNameNumBits);
		byte[] outerDataToHmac = Helpers.concatenate(dupSensitive, _publicPart.getName());
		byte[] outerHmacBytes = CryptoServices.hmac(nameAlg, hmacKey, outerDataToHmac);
	...
	/**
	 * This class encapsulates the data you need to Import a key.
	 * @author pengland
	 *
	 */
...
//			gen.initialize(ecSpec, random);
	...
	*/
	...
	public static TssKey createKey(TPMT_PUBLIC pub)
	{
		if(pub.GetUnionSelector_parameters()== TPM_ALG_ID.RSA.Value)
		{
			...
			int exponent = parms.exponent;
			RsaKeyPair newKey = createRsaKey(keySize, exponent);

			byte[] pubKey = bigIntToTpmInt(newKey.PublicKey, keySize);

			TPMT_PUBLIC retPub = new TPMT_PUBLIC(pub.nameAlg, pub.objectAttributes, pub.authPolicy, pub.parameters, 
					new TPM2B_PUBLIC_KEY_RSA(pubKey));

			TssKey tssKey = new TssKey();
			tssKey.PublicPart = retPub;
		...
		} else if(pub.GetUnionSelector_parameters()== TPM_ALG_ID.ECC.Value) 
		{
			...
			ECCKeyPair p = createECCKey(curve, alg);
			int keySize = ecTpmKeyStrength(curve);

			TPMT_PUBLIC retPub = new TPMT_PUBLIC(
					pub.nameAlg, pub.objectAttributes, pub.authPolicy, pub.parameters, 
					new TPMS_ECC_POINT
					(
							bigIntToTpmInt(p.PublicKey.getXCoord().toBigInteger(),keySize),
							bigIntToTpmInt(p.PublicKey.getYCoord().toBigInteger(), keySize)
							)
					);

			TssKey tssKey = new TssKey();
			tssKey.PublicPart = retPub;
			tssKey.PrivatePart = bigIntToTpmInt(p.PrivateKey, keySize);
			return tssKey;
		}
